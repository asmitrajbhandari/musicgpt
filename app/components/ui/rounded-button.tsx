...
export default function RoundedButton({ 
  ...
  const iconClass = isActive ? 'opacity-100' : (iconOpacity ? `opacity-${iconOpacity}` : '')
  const buttonClass = isActive 
    ...
    : 'flex flex-row items-center gap-2 px-4 py-2 rounded-full bg-transparent border border-[#3a3e42] hover:bg-[#1d2125] transition-all duration-200 active:scale-95'
